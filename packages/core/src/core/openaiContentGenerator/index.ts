...
import {
  ...
  GitHubCopilotOpenAICompatibleProvider,
...
export function determineProvider(
  ...
  // Check for GitHub Copilot provider
  if (GitHubCopilotOpenAICompatibleProvider.isGitHubCopilotProvider(config)) {
    return new GitHubCopilotOpenAICompatibleProvider(
      contentGeneratorConfig,
      cliConfig,
    );
  }
